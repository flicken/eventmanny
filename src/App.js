...
const getAllVisibleEventIds = (state, location, filter) => {
  ...
  switch (location.pathname) {
    case "/new":
      ...
      title = (
        <>
          newly created since <RelativeDate date={recent} />
        </>
      );
      break;

    case "/updates":
      let startOfNextMonth = new DateTime({})
        .plus({ month: 1, days: 3 })
        .startOf("month");
      left = eventsSelectors
        .selectAll(state)
      ...
      title = (
        <>
          updates since <RelativeDate date={recent} />
        </>
      );
      break;
    ...
    default:
      left = eventsSelectors
        .selectAll(state)
        .filter((e) => selectedCalendarIds.has(e.calendarId))
        .map((e) => e.id);
      title = <>all</>;
      break;
  }
  ...
  switch (location.hash) {
    ...
    case "":
      ...
      left.forEach((id) =>
        (state.events.conflicts[id] || new Set()).forEach((i) =>
          allConflicts.add(i)
        )
      );
...
const leftEventsMapStateToProps = (state, props) => {
  const { left } = getAllVisibleEventIds(
    state,
    props.location,
...
const rightEventsMapStateToProps = (state, props) => {
  const { right, rightTitle } = getAllVisibleEventIds(
    state,
    props.location,
    state.visibilityFilter
  );
...
const RightEventList = withRouter(
  connect(rightEventsMapStateToProps)(EventList)
);
...
const SignInError = connect((state) => {
  return { signInError: state.session.signInError };
})(({ signInError }) => {
  if (signInError) {
...
const ConnectedUpdatesFilter = connect(
  (state) => {
    return {
      since: state.visibilityFilter.since,
    };
...
const InnerAppWithRouter = withStyles(useStyles)(
  withRouter(connect(mapStateToProps, mapDispatchToProps)(InnerApp))
);
